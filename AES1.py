# -*- coding: utf-8 -*-
"""
Created on Mon Jan 31 19:28:50 2022
...
def encrypt(plaintext, key, verbose=False):
   ...
   # print_round(0, 'input', matrix_to_text(state), verbose)
    ...
    print_round(round, 'Encription :', matrix_to_text(state), verbose)
...
def decrypt(ciphertext, key, verbose=False):
    """
    Perform the AES inverse cipher routine on the given ciphertext using the given key.
    Specify verbose mode to print out each step of the routine.
    """
    Nb = 4
    Nk = int((len(key) * 4) / 32)
    ...
    #print_round(0, 'ik_sch', get_round_key( w, Nr, Nb), verbose)
   ...
   # print_round(Nr, 'ik_sch', get_round_key(w, 0, Nb), verbose)
    ...
    print_round(Nr,'Decription :', matrix_to_text(state), verbose)
...
def shift_rows(state):
    """
    Cyclically shift the bytes in the last three rows of the state over different
    byte offsets.
    """
...
def inv_shift_rows(state):
    """
    Apply the inverse of ShiftRows by cyclically shifting the bytes in the last three
...
def add_round_key(state, w, round, Nb):
    """ Add a round key to the state via an XOR operation with the words from the key schedule. """
    for c in range(4):
        word = w[round * Nb + c]
        bytes = [(word >> i & 0xff) for i in (24, 16, 8, 0)]
        for r in range(4):
...
if __name__ == "__main__":
    mode = int(input("Enter the Mode (1-128bits) (2-192bits) (3-256) :"))
    if(mode == 1):
        pt = input("Enter the Plain text :")
        key = input("Enter the key of 32 bits :")
        perform_aes_algorithm(pt,key)
    elif mode==2:
        pt = input("Enter the Plain text :")
        key = input("Enter the key of 48 bits :")
        perform_aes_algorithm(pt,key)
    elif mode==3:
        pt = input("Enter the Plain text :")
        key = input("Enter the key of 64 bits :")
        perform_aes_algorithm(pt,key)
